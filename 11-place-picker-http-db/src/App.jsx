...
function App() {
  ...
  useEffect(() => {
    async function loadUserPlaces() {
      ...
      try {
        ...
        setError({message: error.message} || {message: "An error occurred fetching user places."});
      }
      setIsFetching(false);
    }
  ...
  async function handleSelectPlace(selectedPlace) {
    setUserPlaces((prevPickedPlaces) => {
      // optimistic updating: first update the UI, then the backend
      if (!prevPickedPlaces) {
        prevPickedPlaces = [];
      }
  ...
  return (
    <>
      <Modal open={!!errorUpdating} onClose={handleErrorModalClose}>
        {errorUpdating && (
          <ErrorComponent
            title="Error"
      ...
      <main>
        ...
        {!error && <Places
          ...
          loadingPlacesText="Loading your places..."
